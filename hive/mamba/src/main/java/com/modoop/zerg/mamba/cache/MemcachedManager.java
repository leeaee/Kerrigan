package com.modoop.zerg.mamba.cache;

import com.google.common.base.Throwables;
import com.google.common.collect.Maps;
import com.modoop.zerg.taipan.core.memcached.MemcachedCache;
...
public class MemcachedManager implements CacheManager, Initializable, Destroyable
{
    ...
    /**
     * The Memcached cache manager used by this implementation to create cache client.
     */
    ...
    public <K, V> Cache<K, V> getCache(String name) throws CacheException
    {
        ...
        try
        {
            ...
            if (client == null)
            {
                if (ConnectionFactoryBuilder.Protocol.BINARY.toString().equalsIgnoreCase(protocol))
                {
                    client = new MemcachedClient(new BinaryConnectionFactory(), AddrUtil.getAddresses(servers));
                }
                else if (ConnectionFactoryBuilder.Protocol.TEXT.toString().equalsIgnoreCase(protocol))
                {
                    client = new MemcachedClient(AddrUtil.getAddresses(servers));
                }
                else
                {
                    throw new CacheException("Please check the memcached client protocol settings, only BINARY and TEXT supported");
                }
                logger.debug("Create cache with name {} to address {} using protocol {}", name, servers, protocol);
            ...
            return new MemcachedCache<K, V>(client, MemcachedObjectType.valueOf(name).getExpiredTime());
        }
